...
export async function processQuestion(question: string, aiClient: AIClient): Promise<number> {
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content: 'You are a helpful assistant that answers questions with numbers only. If the answer is not a number, convert it to a number that makes sense in the context.',
    },
    {
      role: 'user',
      content: question,
